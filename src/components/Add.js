  ...
  return {
    ...
    explanation: [
      ...
      },
      {
        ...
        value: seq,
        desc:
          "标识从 TCP 源端向 TCP 目的端发送的数据字节流，它表示在这个报文段中的第一个数据字节的顺序号。",
      },
      {
        key: "TCP确认号 [4 bytes]",
        value: acknumber,
      ...
      },
      {
        ...
        value: rst,
        desc:
          "表示从确认号开始，本报文的源方可以接收的字节数，即源方接收窗口大小。",
      },
      {
        key: "TCP校验和 [2 bytes]",
        value: checksum,
      ...
      },
      {
        ...
        value: urgentPointer,
        desc:
          "一个正的偏移量，和顺序号字段中的值相加表示紧急数据最后一个字节的序号。",
      },
    ],
  };
}
    ...
    explanation: [
      {
        ...
        value: IPv,
        desc:
          "高4bit指IP协议的版本，IPv4:0100/IPv6:0110，低4bit指IP首部长度，用[0,15]表示，4字节为一个单位",
      },
      {
        key: "服务类型 [8 bit]",
        value: tos,
      ...
      },
      {
        ...
        value: mf,
        desc:
          "MF=1即表示后面还有分片的数据报，MF=0表示这已是若干数据报片中的最后一个",
      },
      {
        key: "DF标志 3位flag的中间位",
        value: df,
