...
function AddSourceMac(mac) {
  ...
  return (
    ...
    String.fromCharCode(macnum3) +
    String.fromCharCode(macnum4) +
    String.fromCharCode(macnum5)
  );
}
  ...
  return {
    ...
    explanation: [
      ...
      },
      {
        ...
        value: seq,
        desc:
          "[4 bytes] 标识从 TCP 源端向 TCP 目的端发送的数据字节流，它表示在这个报文段中的第一个数据字节的顺序号。",
      },
      {
        key: "TCP确认号",
        value: acknumber,
      ...
      },
      {
        ...
        value: rst,
        desc:
          "用于复位由于主机崩溃或其他原因而出现错误的连接。它还可以用于拒绝非法的报文段和拒绝连接请求。",
      },
      {
        key: "SYN标志",
        value: syn,
      ...
      },
      {
        ...
        value: rst,
        desc:
          "[2 bytes] 表示从确认号开始，本报文的源方可以接收的字节数，即源方接收窗口大小。",
      },
      {
        key: "TCP校验和",
        value: checksum,
      ...
      },
      {
        ...
        value: urgentPointer,
        desc:
          "[2 bytes] 一个正的偏移量，和顺序号字段中的值相加表示紧急数据最后一个字节的序号。",
      },
    ],
  };
}
  ...
  return {
    ...
    explanation: [
      ...
      },
      {
        ...
        value: checksum,
        desc:
          "[2 bytes] 对UDP包头和数据，每16 bit进行反码求和，字节数不为偶数时补0",
      },
    ],
  };
}
    ...
    explanation: [
      {
        ...
        value: IPv,
        desc:
          "[8 bit] 高4bit指IP协议的版本，IPv4:0100/IPv6:0110，低4bit指IP首部长度，用[0,15]表示，4字节为一个单位",
      },
      {
        key: "服务类型",
        value: tos,
      ...
      },
      {
        ...
        value: precedence,
        desc:
          "服务类型中[7:5]bit 定义包的优先级，取值越大数据越重要。\n000 普通 (Routine)\n001 优先的 (Priority)\n010 立即的发送 (Immediate)\n011 闪电式的 (Flash)\n100 比闪电还闪电式的 (Flash Override)\n101 CRI/TIC/ECP\n110 网间控制 (Internetwork Control)\n111 网络控制 (Network Control)",
      },
      {
        key: "d标志",
        value: d,
      ...
      },
      {
        ...
        value: Identifier,
        desc:
          "[2 bytes] 该字段和Flags和Fragment Offest字段联合使用，对较大的上层数据包进行分段（fragment）操作。路由器将一个包拆分后，所有拆分开的小包被标记相同的值，以便目的端设备能够区分哪个包属于被拆分开的包的一部分。",
      },
      {
        key: "MF标志",
        value: mf,
        desc:
          "3位flag的最低位 MF=1即表示后面还有分片的数据报，MF=0表示这已是若干数据报片中的最后一个",
      },
      {
        key: "DF标志",
        value: df,
